...
EE14IOT <- EE14IOT %>% 
 dplyr::rename(EU = 'euroopa Liit', Elec = 'euroopa parlamendi valimised', EP = 'evropský parlament')
...
Nu burde jeg kunne køre en rowsum per country og year


Først prøver vi at lave summeret dataset - det er nice at have.
```{r}
GTsum <- GTperf %>%
  select(geo, year, year_2019, EU, Elec, EP) %>% 
  group_by(geo) %>%
    summarise(                        #evt. med dplyr::summarise ellers plyr::summarise 
      mean_EU = mean(EU), median_EU = median(EU), variance_EU = var(EU),total = sum(EU),
      mean_EP = mean(EP), median_EP = median(EP), variance_EP = var(EP),total = sum(EP),
      mean_Elec = mean(Elec), median_Elec = median(Elec), variance_Elec = var(Elec), 
      total = sum(Elec)) %>% 
  arrange(desc('year_2019'))
  

#summarise(m_hits = mean(total_hits, na.rm = T), v_hits = var(total_hits, na.rm = T))
